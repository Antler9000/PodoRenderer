...
본 리포지토리는 위 방법에 더하여 Frank Luna의 D3D12 입문 도서가 NuGet으로 최신 헤더, 라이브러리, 런타임을 복원하는 방식을 참고하여 사용하였습니다 [1]. 다만 해당 도서가 NuGet으로 관리하지 않고 직접 파일을 포함하던 대상들까지 모두 NuGet으로 복원되도록 확장 적용하였습니다.
| 대상 | 목적 | 파일 |
|---|---|---|
...
### 6.2. 윈도우 창, GUI
- **업데이트 진행 및 중지**
  - IN_GAME 상태에서 ESC를 누르면 PAUSED 상태로 전환할 수 있도록 하기
  - IN_GAME 상태와 접한 다른 상태들에선 게임 로직 업데이트를 멈추기
  - 현재 주 선택 창에서 벗어나거나 작업바로 내려가도 게임 로직 업데이트와 렌더 업데이트가 일어나도록 하기
  - 창이 보이지 않는 경우엔 렌더 업데이트를 멈추기
