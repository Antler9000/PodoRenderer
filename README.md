...
- **GUI 크기 조절 옵션**  
  - 슬라이더로 GUI 크기를 조절할 수 있도록 하기

<br>

- **디버깅 함수 개선**
  - 문제가 일어난 구문의 파일, 라인수, 부모 함수를 알 수 있도록 매크로로 수정하기
...
- **창 크기 및 위치 복원**  
  과거에 사용한 창의 크기와 위치를 기억해두고서 창모드로 복귀하는 경우에 이를 다시 복원하는 로직은 필수적이지 않다고 판단하여, 창모드로 복귀 시 창이 모니터의 66% 크기로 중앙에 위치하도록 간단히 구현하였습니다.

<br>

- **창 크기에 독립적인 렌더 해상도 조절**  
  창 크기에 독립적으로 렌더 해상도를 설정할 수 있다면 적은 연산 비용으로 넓은 화면을 플레이할 수 있다는 장점이 있지만, 필수적인 기능은 아니기에 제외하였습니다. 표준적인 게이밍 사양에서 오리지널 해상도로 플레이할 수 있도록 최적화하는데 더 집중하려고 합니다.

<br>

- **자체 GUI 기능**  
  자체 GUI가 있으면 게임의 분위기를 더 잘 반영할 수 있겠지만, GUI를 직접 구현하는 것은 큰 시간이 필요할 것 같아 Dear ImGui를 통해 구현하기로 하였습니다. 게임의 몰입도를 살릴 겸 인게임에서 GUI를 최소화하고, 설정과 종료와 같이 꼭 필요한 외부 GUI만을 간단히 제공하려고 합니다.
