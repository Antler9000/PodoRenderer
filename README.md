# PodoRenderer
D3D12, Win32, ImGui를 이용하여 구현 중인 렌더링 엔진 리포지토리입니다.

최소한의 렌더링 엔진 형태를 갖추기 위한 탐색 구현 과정에 있습니다.
...
## 6. 구현 예정
- **렌더링 기초**
  - 간단하고 정적인 자원 관리, 서술자 관리, 루트 서명, PSO, 셰이더
  - 테스트 월드
  - 자원 준비와 로딩 버튼 동기화
  - SDR 감마 디코딩, 인코딩
  - 조명 효과, 텍스처 매핑, 노말 매핑
  - 카메라 조작 기능(좌클릭 마우스 회전, 키보드 이동, 휠 이동), 조작 중 커서 숨기기

<br>

- **렌더링 구조 개선**
  - CPU 처리 병렬화     : 멀티스레드
  - 로딩할 대상 선별    : 월드 자원 로딩 정책, 정적 LOD
  - 그리기 대상 선별    : 오브젝트 테이블, 절두체 선별
  - 그리기 대상 최신화  : 프레임 자원, 서술자 힙 2차원화
  - 루트 서명 크기 축소 : 셰이더 자원 테이블을 이용한 간접 바인딩
  - 그리기 명령 수 축소 : 인스턴싱
  - 로딩 축소           : 동적 LOD, 메시 셰이딩 옵션

<br>

- **렌더링 기능 추가**
  - 조명    : 그림자 매핑, 반사 입방체 매핑, 굴절 입방체 매핑, SSAO, 레이 트레이싱 옵션
  - 환경    : 지형 변위 매핑, 수면 변위 매핑, 스카이박스 입방체 매핑
  - 이펙트  : (미정)
  - 테스트  : 데스트 씬 수정
...
- **렌더링 성능 개선**
  ...
  - 아웃 패스 최적화 : PSO 변경 횟수, 루트 서명 크기, 루트 인자 변경 횟수, 그리기 명령 횟수 절약
...
- **제시 기능 개선**
  ...
  - `DXGI_FORMAT_R10G10B10A2_UNORM`, `DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020` 색역 매핑, PQ 전달 함수 셰이더
